...
# @description Pull data from `rd3_freeze*_subject` (taken from original script)
# @param release string containing the entity name, i.e. `<package>_<entity>` 
# @return list of dicts
def get_freeze_subjects(release):
    attribs = 'id,subjectID,solved,remarks,recontact,contact,remarks'
    
    # get cases where retracted status is missing/blank
    retracted = rd3.get(
        release,
        attributes = attribs,
        batch_size = 1000,
    ...
    print(
        'Total number of subjects where `retracted = null`: {}'
        .format(len(retracted))
    )
    
    # get cases where status is "no" or "unknown"
    retracted_no_unknown = rd3.get(
        release,
        attributes = attribs,
        batch_size = 1000,
        q = "retracted=in=('N','U')"
    ...
    print(
        'Total number of subjects where `retracted in (No, Unknown)'
        .format(len(retracted_no_unknown))
    )
...
# @description determine if the any of the solved status variables have changed (per subject)
# @param data list of dictionaries containing processed freeze subject metadata
# @param subjects
...
# @param data list of dictionaries containing portal data (i.e., solved status table)
# @param subjects returned object of `process_portal_status`
...
def import_data(entity, data):
    ...
    # update subject table: contact information
    print('Updating contact information...')
    update = rd3.batch_update_one_attr(
        entity = entity,
        attr = 'contact',
        values = data['updateContact']
    )
    print(update)
    
    # update subject table recontact status
    print('Updating recontact information...')
    update = rd3.batch_update_one_attr(
        entity = entity,
        attr = 'recontact',
        values = data['updateRecontact']
    )
    print(update)
    
    # update subject table: solved status
    print('Updating solved status...')
    update = rd3.batch_update_one_attr(
        entity = entity,
        attr = 'solved',
        values = data['updateSolved']
    )
    print(update)
    
    # update subject table: date solved
    ...
    # update subject table: remarks
    print('Updating remarks...')
    update = rd3.batch_update_one_attr(
        entity = entity,
        attr = 'remarks',
        values = data['updateRemark']
    )
    print(update)
...
# process RD3 releases (i.e., freezes)
releases = ['freeze1', 'freeze2']
for release in releases:

    # print initial message and build entity IDs
    print('Processing portal data for {}'.format(release))
    releaseName = 'rd3_' + release + '_subject'

    # Pull release subject metadata
    print('Fetching subject entities...')
    freeze_subjects = get_freeze_subjects(
        release = releaseName
    )

    # process freeze subject data 
    print('Processing subject data...')
    freeze_subjects_processed = process_freeze_subjects(
        data = freeze_subjects
    )

    # process freeze data determine which cases have changed
    freeze = process_portal_status(
        data = portal_data,
        subjects = freeze_subjects_processed
    )

    # print descriptives for each freeze: What needs to be updated?
    ...
    # Update solved status table: history
    print('Updating portal history...')
    update = rd3.batch_update_one_attr(
        entity = 'rd3_portal_recontact_solved',
        attr = 'history',
        values = freeze['updatePtlHistory']
    )
    print(update)

    # update solved status table: processed status 
    print('Updating portal status...')
    update = rd3.batch_update_one_attr(
        entity = 'rd3_portal_recontact_solved',
        attr = 'process_status',
        values = freeze['updatePtlStatus']
    )
    print(update)

    # update solved status table: remarks
    print('Updating portal remarks...')
    update = rd3.batch_update_one_attr(
        entity = 'rd3_portal_recontact_solved',
        attr = 'remark',
        values = freeze['updatePtlRemark']
    )
    print(update)


# Lastly, check if all New portal records are processed
post_portal_data = rd3.get(entity = 'rd3_portal_recontact_solved', q = 'process_status=="N"')
if len(post_portal_data) > 0:
