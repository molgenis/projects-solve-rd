...
This script is unfninshed. This is based on a copy from rd3/emx2_mapping.py. 
The aim was to structure the code a bit more and remove some duplicate code. 
Due to pressing time, this is unfninshed. 
mapping Individuals and Pedigree is fininshed. Pedigree members almost. The other tables are not yet mapped. 
All original code is present in the script, commented out below the new part. 
...
class map_solve_rd:
    ...
    def __init__(self, subjects: list = None, subjects_info: list = None):
    ...
    def map_individuals(self):
        ...
        self.emx2_individuals = []
        self.emx2_pedigree = []
        self.emx2_pedigree_members = []
        self.emx2_clinical_observations = []
        self.emx2_individual_consent = []
        ...
        for subject in self.subjects:
            ...
            self.individuals[subject_id]['year of birth'] = "".join(map(str, match))
    ...
    def map_pedigree(self):
        for subject in self.subjects:
            ...
            if 'fid' in subject:
                fid = subject['fid']
                self.individuals[subject_id]['pedigree'] = subject['fid']
                # to make this work the fid also needs to be added to the pedigree table in emx2.
                # check for uniqueness
                if not any(entry['id'] == subject['fid'] for entry in self.pedigree.values()):
                    self.pedigree[fid] = {'id': fid}

    def pedigree_entry(self, pedigree: str, individual: str, affected: bool, relative: str, relation: str):
        """This function creates a new pedigree entry based on the input"""
        new_entry = {}
        new_entry['pedigree'] = pedigree
        new_entry['individual'] = individual
        new_entry['affected'] = affected
        new_entry['relative'] = relative
        new_entry['relation'] = relation

        return new_entry

    def map_pedigree_members(self):
        """This function maps the pedigree members based on mid and pid in solve-rd.
            Mapping pedigree. 
            ...
            If an individual has a family ID, but no other information, the person is added to the table
              with itself as the relative. This happens later in the code.
        ...
        for subject in self.subjects:
           ...
           # if 'fid' in subject: 
            fid = subject.get('fid')
            if fid and ('pid' in subject or 'mid' in subject):

                new_pedigree_members_entry = self.pedigree_entry(pedigree=fid,
                                                                individual= subject_id,
                                                                affected=subject.get('clinical_status'),
                                                                relative=subject_id,
                                                                relation='Patient')
                if fid not in self.pedigree_members:
                   self.pedigree_members[fid] = []
                self.pedigree_members[fid].append(new_pedigree_members_entry)
                # keep track of the patients
                self.patients.append(subject_id)
            ...
            if fid:
                for subj in subjects:
                    ...
                    # Map Biological Father
                        ...
                        self.track_fathers[subj['subjectID']] = subject_id
                        # if 'mid' in subject or 'pid' in subject:
                            # print(f'Individual has a parent and is a parent for ind {subject['subjectID']}')
                    else: 
                        relative = ""
                        relation = ""
                    new_pedigree_members_entry = self.pedigree_entry(pedigree=pedigree,
                                                                    individual=individual,
                                                                    affected=affected,
                                                                    relative=relative,
                                                                    relation=relation)
                    if fid not in self.pedigree_members:
                       self.pedigree_members[fid] = [fid]
                    if new_pedigree_members_entry.get('relative'): # only append new entry if individual has a parent
                        self.pedigree_members[fid].append(new_pedigree_members_entry)
                    
                # Map maternal grandparents
                if subject['subjectID'] in self.patients and subject_id in self.track_mothers.values():
                    # Map Biological Maternal Grandmother
                    if 'mid' in subject:
                        # get the identifier for the grandchild
                        grandchild_ids = [child_id for child_id, value in self.track_mothers.items() if value == subject_id]
                        for grandchild_id in grandchild_ids:
                            ...
                            if clinical_status:
                                affected = clinical_status[0]
                            new_pedigree_members_entry = self.pedigree_entry(pedigree=fid,
                                                                            individual=subject['mid']['subjectID'],
                                                                            affected=affected,
                                                                            relative=grandchild_id,
                                                                            relation="Biological Maternal Grandmother")
                            #new_pedigree_members_entry['pedigree'] = subject['fid']
                    ...
                    if 'pid' in subject:
                        ...
                        grandchild_ids = [child_id for child_id, value in self.track_mothers.items() if value == subject_id]
                        for grandchild_id in grandchild_ids:
                            new_pedigree_members_entry = {} # TODO: change these to use the pedigree_entry as well 
                            new_pedigree_members_entry['pedigree'] = subject['fid']
                ...
                if subject_id in self.patients and subject_id in self.track_fathers.values():
                    # Map Biological Paternal Grandmother
                    if 'mid' in subject:
                        # get the identifier for the grandchild
                        grandchild_ids = [child_id for child_id, value in self.track_fathers.items() if value == subject_id]
                        for grandchild_id in grandchild_ids:
    ...
    def get_results(self):
        normalized = []
        for key,value in self.pedigree_members.items():
            if not value:
                normalized.append({'pedigree': key, 'individual': '', 'affected': False, 'relative': '', 'relation': ''})
            else: 
                for item in value: 
                    if isinstance(item, dict):
                        pedigree = item.get('pedigree', key)
                        individual = item.get('individual', key)
                        affected = item.get('affected', key)
                        relative = item.get('realtive', key)
                        relation = item.get('relation', key)
                        normalized.append({'pedigree': pedigree,
                                            'individual': individual,
                                            'affected': affected,
                                            'relative': relative,
                                            'relation': relation})
        return pd.DataFrame(normalized) # TODO: only the patients are displayed in this df
...
mapper = map_solve_rd(subjects, subjects_info)
mapper.map_individuals()
mapper.map_pedigree()
mapper.map_pedigree_members()
